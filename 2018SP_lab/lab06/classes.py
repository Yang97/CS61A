...
class Player(object):
    ...
    def talk_to(self, person):
        ...
        elif person in self.place.characters.keys():
            instance = self.place.characters[person]
            print(instance.name, "says:", instance.message)
        else:
    ...
    def unlock(self, place):
        ...
        if type(place) != str:
            ...
            return
        place = self.place.exits[place][0]
        if not place.locked:
            print(place.name, "is already unlocked!")
        else:
            for item in self.backpack:
                if type(item) == Key and item.use(place):
                    place.locked = False
...
class Key(Thing):
    def use(self, place):
        if self.description == 'A Key to unlock all doors.':
            return True
        elif self.name == place.name:
            return True
        else:
            return False
