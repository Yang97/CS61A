...
def flatten(lst):
    ...
    [1, 1, 1, 1, 1, 1]
    """
    def flatten1(lst, index, acc, length):
        if index < length:
            elem = lst[index]
            if type(elem) == list:
                acc += flatten1(elem, 0, [], len(elem))
                return flatten1(lst, index+1, acc, length)
            else:
                acc.append(elem)
                return flatten1(lst, index+1, acc, length)
        return acc
    return flatten1(lst, 0, [], len(lst))
...
def merge(lst1, lst2):
    ...
    [2, 4, 5, 6, 7]
    """
    index1, index2, len1, len2, lst = 0, 0, len(lst1), len(lst2), []
    for x in range(0, len1 + len2):
        if index1 == len1 and index2 == len2:
            return lst
        elif index1 == len1:
            return lst + lst2[index2:]
        elif index2 == len2:
            return lst + lst1[index1:]
...
def replace_elem(lst, index, elem):
    ...
    return [lst[i] if i != index else elem for i in range(0, len(lst))]
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(0, max_rows):
        for column in range(0, max_cols):
            string += str(get_piece(board, row, column))
            string += " "
        string.strip()
        s = str(list(string).pop())
        print(s)
        string = ""
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    False
    """
    for column in range(0, max_cols - num_connect + 1):
        acc = 0
        for x in range(0, num_connect):
            if get_piece(board, row, column + x) == player:
                acc += 1
            if acc == num_connect:
                return True
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    False
    """
    for row in range(0, max_rows - num_connect + 1):
        acc = 0
        for x in range(0, num_connect):
            if get_piece(board, row + x, col) == player:
                acc += 1
            if acc == num_connect:
                return True
